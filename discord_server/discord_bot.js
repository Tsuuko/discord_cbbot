  ...
  ");
    ...
    var cb_attack = function(message) {
        if (cb_command.length === 5 && cb_command[2] === "-u") {
            ...
            // attack3の絵文字の場合（内部IDは!cb test emojiid :attack3:で確認）
            if (cb_command[0] == "<:attack3:638061398357966849>") {
                cb_attack_post(message.member.nickname, 3);
            } else {
    ...
    var test = function(message) {
        // 絵文字内部ID確認（https://nichiafi.blogspot.com/2018/02/discordbot.html）
        if (cb_command[2] === "emojiid") {
            message.reply("```" + cb_command[3] + "```");
        }
    }
    ...
    // !cbコマンドか絵文字の場合
    if (message.content.startsWith("!cb") || message.content.startsWith("<:")) {
        // コマンドをスペースで切り出し
        var cb_command = message.content.split(" ");
        //
        if (cb_command[1] === "test") {
            test(message);
        }
        // 起動コマンドのみ
        if (message.content.startsWith("!cb") && cb_command.length === 1) {
        ...
        // クラバト凸コマンド
        else if (cb_command[1] === "attack" || message.content.startsWith("<:attack")) {
            cb_attack(message);
            return 0;
        }
        // クラバトユーザー登録コマンド
